    ...
    <section id="yorumlar" className="scroll-mt-20 border-b border-border">
      <div className="mx-auto max-w-6xl px-5 py-16 md:py-24">
        ...
        <ul className="mt-12 grid grid-cols-1 gap-x-10 gap-y-12 md:grid-cols-3">
          {items.map((t, i) => (
            <Reveal as="li" key={t.id} className={cn("border-t border-border pt-6", i % 3 === 1 && "md:mt-12")} delay={staggerDelay(i % 3, 0.08)}>
              ...
              <p className="label mt-5 text-muted-foreground">{t.name}</p>
            </Reveal>
          ))}
        </ul>
      </div>
