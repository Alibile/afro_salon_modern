...
**Interfaces:**
- Navbar (referans yapısı): landing'de `SiteHeader` yerine `SiteNav` (yalnızca `/`), yapışkan (`sticky top-0`), zemin `bg-background/90 backdrop-blur`, altta ince çizgi. Sol: salon adı display fontla (logo görevi, `/`'a bağlantı). Orta: Ana Sayfa (`/`), Hakkımızda (`#hakkimizda`), Hizmetler (`#hizmetler`), Ekip (`#ekip`), Galeri (`#galeri`), Yorumlar (`#yorumlar`), İletişim (`#iletisim`). Sağ: sosyal ikonlar (Instagram, Facebook, WhatsApp — yalnızca ayarlarda dolu olanlar, lucide, `aria-label`), tema anahtarı, oturum durumuna göre "Giriş" / "Randevularım" / "Panel" metin bağlantısı, ve DOLU terracotta "Randevu al" butonu (`/randevu`). Mobilde (md altı): logo + "Randevu al" + hamburger; açılınca tam genişlik menü (client component, `aria-expanded`, Escape ve bağlantı tıklayınca kapanır). Bölüm `<section id=...>` ve `scroll-margin-top: 5rem`. Aktif bölüm vurgusu gerekmez.
...
- Footer (referans yapısı, terracotta zemin + AfroPattern 0.12 korunur), masaüstünde 4 sütun / mobilde tek sütun:
  1. **Marka:** salon adı (display), bir satır slogan ("Aynı gün randevu. Yarına değil, bugüne." korunur), altında sosyal ikon satırı (Instagram, Facebook, WhatsApp; yalnızca dolu olanlar).
  2. **Site:** Ana Sayfa, Hakkımızda, Hizmetler, Ekip, Galeri, Yorumlar, İletişim (aynı `#` bağlantılar).
  3. **Hızlı bağlantılar:** Randevu al; oturum durumuna göre Giriş + Kayıt ol / Randevularım + (personel ise) Panel + Çıkış formu (mevcut davranış).
  4. **İletişim:** adres, tıklanabilir telefon (`tel:`), e-posta (`mailto:`), bugünkü açık/kapalı durumu (`getShopStatus` metni).
  En altta ince çizgi ve telif satırı: "© {yıl} {salon adı}. Tüm hakları saklıdır." Blog sütunu YOK (site blog içermiyor).
